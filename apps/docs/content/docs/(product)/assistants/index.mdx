...
- **Flows** is where you define the rules that control how the assistant responds.
  See [Flows](/flows).
- **Tools & Skills** lets you switch the assistant's built-in tools on or off, add
  your own HTTP tool for the assistant to call, and attach reusable skills —
  playbooks your organization writes once and shares across assistants.
- **Goals** holds the questions you expect the assistant to answer correctly. They
  are re-checked on a schedule, and a goal that starts failing raises an alert.
...
8. Once it is live, add a few **Goals** so you find out when a good answer stops
   being one.
