...
A Flow triggers on a Visitor message, on a widget event, or on an inbound HTTP request. The widget events are page load, time on page, and chat opens. The trigger decides which conditions and which actions the Flow may use. See [HTTP and webhooks](/flows/http) for the inbound trigger.
