...
Answers conversational courtesy — a greeting, a thank you, a goodbye — without
searching anything. A visitor who types "hi" has not asked a question, so there is
nothing to look up, and searching anyway only makes the reply slower. This action
writes one short sentence or two in the visitor's own language, in your assistant's
voice, and invites the real question. It shows no thinking steps and no sources,
because it used neither.

It belongs to the built-in **Basic Interaction** flow, which sits first in your
flow list. You can disable that flow if you would rather every message go through
your knowledge, reorder it against your own flows, or type an exact reply into the
action to pin the wording instead of having it generated.
