...
L'ingestion de FAQ crée un Concept organisé avec la provenance de la question et
de la réponse. L'ingestion de fichiers et de sites Web préserve l'objet Source
d'origine, le cas échéant.

## Récupération

Le moteur d'exécution peut récupérer des Concepts via la recherche vectorielle
et le repli lexical. Le mode Graphique peut ajouter une navigation à travers les
...
limites du moteur d'exécution. Il n'effectue pas de requêtes sur le Web public.
