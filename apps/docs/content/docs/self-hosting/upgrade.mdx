...
With an [external database](/self-hosting/external-database), the same command applies the pending migrations to your managed Postgres. The `provision` service runs first and confirms the roles and schemas. Take a provider snapshot before the upgrade.
