...
You can replace the Postgres container with a managed Postgres you already own. See [External database](/self-hosting/external-database). The other services stay the same.
...
With an [external database](/self-hosting/external-database), the provider keeps the database backups. The `storage-data` volume still lives on your host. Back up the volume together with the database, and restore both to the same point in time.
