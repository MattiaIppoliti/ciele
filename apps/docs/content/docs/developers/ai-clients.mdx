...
The hosted endpoint has no read-only switch: its permissions are the Role of the key. A Viewer key is not a read-only agent, though — reviewing conversations counts as curation, so it can pin a conversation, leave feedback and delete one. Use a local stdio server with `CIELE_MCP_READ_ONLY=1` when the agent must not change anything at all.
