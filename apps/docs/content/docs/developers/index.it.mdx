...
Per scoprire l'operazione sottostante a un controllo della console, apri il
...
<Mermaid
  title="Tre client, un'API, un set di operazioni"
  chart={`
flowchart TB
  CLI["Ciele CLI"]
...
Il servizio in hosting utilizza `https://ciele.app` per impostazione
predefinita. Un'installazione self-hosted utilizza il proprio origin pubblico.
...
L'API attuale include Assistenti, Flussi, Conoscenza, pubblicazione,
...
dell'avatar e la creazione di un miglioramento rimangono disponibili solo dalla
console.
