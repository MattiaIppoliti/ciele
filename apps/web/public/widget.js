...
(function () {
  ...
  // Published Style section values (launcher geometry, icons, window size).
  // Defaults mirror apps/web/src/lib/widget-style.ts; the config fetch below
  // overwrites them with whatever the org published.
  var st = {
    size: 56,
    radius: 100, // px, capped at half the size (100 on 56 = circle)
    padBottom: 24,
    padRight: 24,
    corner: position === "left" ? "bottom-left" : "bottom-right",
    showOnMobile: true,
    focusRing: "",
    launcherIcon: "",
    closeIconUrl: "",
  ...
  function horizSide() {
    return st.corner.indexOf("left") !== -1 ? "left" : "right";
  }
  function vertSide() {
    return st.corner.indexOf("top") === 0 ? "top" : "bottom";
  }
  ...
  var FRAME_TRANSITION =
    ...
    ",bottom .42s " + EASE + ",top .42s " + EASE +
    ",right .42s " + EASE + ",left .42s " + EASE +
  ...
  function applyStyles() {
    var horiz = horizSide();
    var vert = vertSide();
    var radius = Math.min(st.radius, st.size / 2);
    // The Style section's Mobile toggle: launcher hidden on small screens.
    var mobileHidden = !st.showOnMobile && window.innerWidth < 768;
    button.style.cssText =
      "position:fixed;" + vert + ":" + st.padBottom + "px;" +
      horiz + ":" + st.padRight + "px;" +
      "z-index:2147483000;width:" + st.size + "px;height:" + st.size + "px;" +
      "border-radius:" + radius + "px;border:none;" +
      "cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,.25);background:" + color +
      ";color:#fff;display:" + (mobileHidden ? "none" : "flex") +
      ";align-items:center;justify-content:center;transition:transform .15s;";
    var bg = frameBackground();
    var display = "display:" + (state.open && !mobileHidden ? "block" : "none") + ";";
    ...
    if (state.fullscreen) {
      ...
      // Floating rounded card above (or below, for a top corner) the launcher,
      // sized by the published window size and capped to the viewport.
      var clearance = st.padBottom + st.size + 12;
      size =
        "width:min(" + st.winW + "px,calc(100vw - 48px));" +
        "height:min(" + st.winH + "px,calc(100vh - " + (clearance + 28) + "px));";
      offset = vert + ":" + clearance + "px;" + anchor + st.padRight + "px;";
  ...
  // A published custom icon is a base64 data: URL (built by FileReader in the
  // Style editor, so it never contains a quote); scaled to half the launcher.
  function customIcon(url) {
    var px = Math.round(st.size / 2);
    return (
      '<img src="' + url.replace(/"/g, "") + '" alt="" style="width:' +
      px + "px;height:" + px + 'px;object-fit:contain;pointer-events:none;"/>'
    );
  }

  function render() {
    button.innerHTML = state.open
      ? (st.closeIconUrl ? customIcon(st.closeIconUrl) : closeIcon)
      : (st.launcherIcon ? customIcon(st.launcherIcon) : chatIcon);
  ...
  button.addEventListener("focus", function () {
    warmFrame();
    // The Style section's focus ring, keyboard focus only.
    if (st.focusRing && button.matches(":focus-visible")) {
      button.style.outline = "3px solid " + st.focusRing;
  ...
  fetch(base + "/api/widget/" + encodeURIComponent(assistantId) + "/config")
    ...
    .then(function (config) {
      ...
      if (!script.getAttribute("data-color")) {
        // The Buttons color is the launcher's; brandColor is its fallback.
        color = cs.buttonColor || cs.brandColor || color;
      }
      if (!script.getAttribute("data-position")) {
        // The four-corner pick wins over the legacy left/right position.
        if (cs.corner) st.corner = cs.corner;
        else if (cs.position === "left") st.corner = "bottom-left";
      }
      ...
      st.size = num(cs.buttonSize, st.size, 32, 120);
      st.radius = num(cs.buttonRadius, st.radius, 0, 100);
      st.padBottom = num(cs.paddingBottom, st.padBottom, 0, 200);
      st.padRight = num(cs.paddingRight, st.padRight, 0, 200);
      st.winW = num(cs.windowWidth, st.winW, 280, 800);
      st.winH = num(cs.windowHeight, st.winH, 320, 1200);
      if (cs.showOnMobile === false) st.showOnMobile = false;
      if (typeof cs.focusRingColor === "string") st.focusRing = cs.focusRingColor;
      // Only data: URLs render, anything else in the config is ignored.
      if (typeof cs.launcherIcon === "string" && cs.launcherIcon.indexOf("data:image/") === 0) {
        st.launcherIcon = cs.launcherIcon;
      }
      if (typeof cs.closeIcon === "string" && cs.closeIcon.indexOf("data:image/") === 0) {
        st.closeIconUrl = cs.closeIcon;
      }
      if (!st.focusRing) st.focusRing = color;
      render();
