import { createSkillOp, listSkillsOp } from "@ciele/ops";
import { apiError } from "@/lib/api-v1/http";
import { runApiOperation } from "@/lib/api-v1/run";

export async function GET(request: Request) {
  const outcome = await runApiOperation(request, listSkillsOp, {});
...
export async function POST(request: Request) {
  ...
  const outcome = await runApiOperation(request, createSkillOp, body);
