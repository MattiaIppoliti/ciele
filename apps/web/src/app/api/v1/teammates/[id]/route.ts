import { deleteTeammateOp, getTeammateOp, updateTeammateOp } from "@ciele/ops";
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const outcome = await runApiOperation(request, updateTeammateOp, { id, patch });
  return outcome instanceof Response ? outcome : Response.json(outcome.result);
}

export async function DELETE(request: Request, { params }: Params) {
  const { id } = await params;
  const outcome = await runApiOperation(request, deleteTeammateOp, { id });
