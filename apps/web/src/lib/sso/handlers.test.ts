...
const transient = {
  ...
  redirectUri: "https://ciele.app/api/sso/entra/callback",
};
...
describe("startSsoFlow", () => {
  it("302s to the IdP and sets a sealed transient cookie", async () => {
    ...
    const req = new NextRequest(
      `https://ciele.app/api/sso/entra/start?assistantId=${ASSISTANT}`
    );
  ...
  it("captures a same-origin returnTo but drops an off-origin one", async () => {
    ...
    const same = await startSsoFlow(
      new NextRequest(
        `https://ciele.app/api/sso/entra/start?assistantId=${ASSISTANT}&returnTo=${encodeURIComponent("/widget/a1")}`
      ),
      "entra"
    );
    expect(openTxn(same.cookies.get("sso_txn")?.value)?.returnTo).toBe(
      "https://ciele.app/widget/a1"
    );

    const evil = await startSsoFlow(
      new NextRequest(
        `https://ciele.app/api/sso/entra/start?assistantId=${ASSISTANT}&returnTo=${encodeURIComponent("https://evil.example/x")}`
      ),
      "entra"
    );
...
describe("handleSsoCallback", () => {
  function callbackRequest(stateParam: string, returnTo?: string) {
    ...
    return new NextRequest(
      `https://ciele.app/api/sso/entra/callback?code=auth-code&state=${stateParam}`,
  ...
  it("redirects to returnTo in the top-level (no-opener) flow", async () => {
    ...
    const res = await handleSsoCallback(
      callbackRequest("state-1", "https://ciele.app/widget/a1"),
      "entra"
    );
    const html = await res.text();
    // The result page carries the returnTo for the no-opener branch.
    expect(html).toContain("https://ciele.app/widget/a1");
  ...
  it("fails closed when the transient cookie is absent", async () => {
    ...
    const req = new NextRequest(
      "https://ciele.app/api/sso/entra/callback?code=c&state=state-1"
    );
...
describe("logoutSsoFlow", () => {
  it("clears the gate cookie and redirects to a same-origin returnTo", async () => {
    const req = new NextRequest(
      "https://ciele.app/api/sso/entra/logout?returnTo=/widget/a1"
    );
    const res = await logoutSsoFlow(req);
    expect(res.status).toBe(307);
    expect(res.headers.get("location")).toBe("https://ciele.app/widget/a1");
    expect(res.cookies.get("sso_gate")?.value).toBe("");
  });

  it("refuses an off-origin returnTo (no open redirect)", async () => {
    const req = new NextRequest(
      "https://ciele.app/api/sso/entra/logout?returnTo=https://evil.example/steal"
    );
    const res = await logoutSsoFlow(req);
