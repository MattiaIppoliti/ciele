...
export async function claimRelayJob(input: {
  ...
  // The expired-job sweep rides the same cadence as the heartbeat: both are
  // maintenance, neither needs the 1s hot rate, and an expired prompt now
  // lingers at most HEARTBEAT_MS longer than it did when every claim swept.
  const maintenanceDue = heartbeatIsDue(device, input.providers);
  if (maintenanceDue) {
