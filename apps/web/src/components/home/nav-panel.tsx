...
function DropdownCard({ card, onNavigate }: { card: PanelCard; onNavigate: () => void }) {
  return (
    <Link
      ...
      className="press group/card bg-muted/40 hover:bg-muted/70 relative flex h-full w-56 shrink-0 flex-col overflow-hidden rounded-[calc(var(--radius-3xl)-0.5rem)] border p-4 duration-200"
    >
    ...
    >
      {docsAreas.map(({ name, href, Icon }) => (
        <Link
          ...
          title={name}
          onClick={onNavigate}
          className="press-control text-muted-foreground/70 hover:bg-muted hover:text-foreground flex size-11 items-center justify-center rounded-xl duration-150"
        >
  ...
  return (
    <div>
      <button
        ...
        onClick={onToggle}
        aria-expanded={open}
        className="press-text text-muted-foreground aria-expanded:text-foreground flex w-full items-center justify-between gap-3 text-3xl font-light duration-150"
      >
        <span>{item.name}</span>
        <ChevronDown
              ...
              {links.map((child, index) => (
                ...
                >
                  <Link
                    ...
                    rel={child.external ? "noopener noreferrer" : undefined}
                    onClick={onNavigate}
                    className="press-text text-muted-foreground hover:text-foreground block py-0.5 text-lg font-light duration-150"
                  >
                    {child.name}
                  </Link>
                </motion.li>
          ...
          <Reveal delay={0.05 + i * 0.08}>
            {item.columns ? (
              ...
              <Link
                ...
                onClick={onNavigate}
                className="press-text text-muted-foreground hover:text-foreground block text-4xl font-light duration-150"
              >
                <span>{item.name}</span>
              </Link>
            )}
