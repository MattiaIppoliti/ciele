...
interface Visual1Props {
  ...
  gridColor?: string;
  /** Replaces the default hover caption in Layer4 (slides in from the top on
   * card hover). */
...
export function Visual1({
  ...
  return (
    <div
      aria-hidden
      className="relative h-[180px] w-[356px] shrink-0 overflow-hidden rounded-t-lg"
    >
...
const EllipseGradient = ({ color }: { color: string }) => {
  return (
    <div className="absolute inset-0 z-[5] flex h-full w-full items-center justify-center">
      ...
      >
        <rect width="356" height="180" fill="url(#paint_v1)" />
        <defs>
          <radialGradient
            id="paint_v1"
            cx="0"
...
const Layer1 = ({ color, secondaryColor }: LayerProps) => {
  return (
    <div className="absolute left-0 top-0 z-[6] transform transition-transform duration-500 ease-[cubic-bezier(0.6,0.6,0,1)] group-hover/animated-card:translate-x-[-50%]">
      <svg
        className="w-[712px]"
        viewBox="0 0 712 180"
        fill="none"
...
const Layer2 = ({ color }: LayerProps) => {
  return (
    <div className="absolute left-[-1px] top-0 h-full w-[356px]">
      <svg
      ...
      >
        ...
        <defs>
          <clipPath id="clip0_v1">
            <rect width="356" height="180" fill="white" />
          </clipPath>
        </defs>
      </svg>
      <div className="absolute inset-0 z-[3] transform bg-gradient-to-r from-transparent from-0% to-white to-15% transition-transform duration-500 ease-[cubic-bezier(0.6,0.6,0,1)] group-hover/animated-card:translate-x-full dark:to-black"></div>
    </div>
  );
};
  ...
  return (
    ...
    >
      <div className="flex shrink-0 items-center rounded-full border border-zinc-200 bg-white/25 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0 dark:border-zinc-800 dark:bg-black/25">
        ...
        <span className="ml-1 text-[10px] text-black dark:text-white">Docs</span>
      </div>
      <div className="flex shrink-0 items-center rounded-full border border-zinc-200 bg-white/25 px-1.5 py-0.5 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0 dark:border-zinc-800 dark:bg-black/25">
        <div className="h-1.5 w-1.5 rounded-full bg-[var(--secondary-color)]" />
        <span className="ml-1 text-[10px] text-black dark:text-white">FAQs</span>
      </div>
    </div>
  );
};

const Layer4 = ({ label }: { label?: React.ReactNode }) => {
  return (
    <div className="group relative h-full w-[356px]">
      <div className="absolute inset-0 z-[7] flex max-w-[356px] -translate-y-full items-start justify-start bg-transparent p-4 transition-transform duration-500 ease-[cubic-bezier(0.6,0.6,0,1)] group-hover/animated-card:translate-y-0">
        {label ? (
          <div className="opacity-0 transition-opacity duration-500 ease-[cubic-bezier(0.6,0.6,0,1)] group-hover/animated-card:opacity-100">
            {label}
          </div>
        ) : (
          <div className="rounded-md border border-zinc-200 bg-white/25 p-1.5 opacity-0 backdrop-blur-sm transition-opacity duration-500 ease-[cubic-bezier(0.6,0.6,0,1)] group-hover/animated-card:opacity-100 dark:border-zinc-800 dark:bg-black/25">
