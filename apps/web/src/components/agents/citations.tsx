  ...
  return (
    ...
    >
      {favicon && failedUrl !== favicon ? (
        ...
        // A website whose favicon didn't load still reads as a website.
        <Globe2 className="size-3.5" />
      ) : (
        // No URL = an attached document in the knowledge base, not a page.
  ...
  const content = (
    <>
      ...
      <span className="flex shrink-0 items-center gap-1.5">
        <span className="grid size-5 place-items-center rounded-md bg-foreground/[0.05] text-2xs font-semibold tabular-nums text-muted-foreground">
          {index}
        </span>
        {citation.url ? (
