...
export function useAgentCodeTokens(
  ...
  // Cache hits resolve during render (the documented derived-state adjustment
  // pattern) so the effect only ever schedules the async highlight.
  if (result?.key !== key && cached) {
    setResult({ key, code, language, lines: cached });
  }

  useEffect(() => {
    if (tokenCache.get(key)) return;
