...
const nextConfig: NextConfig = {
  ...
  experimental: {
    // Client Router Cache lifetime for dynamic segments (Next 16 default: 0 —
    // every soft navigation refetches the full RSC payload). Every admin page
    // is force-dynamic, so 0 makes back-and-forth navigation between admin
    // screens pay the whole session + org + shell round-trip chain each time.
    // 30s keeps recently visited pages instant; Server Actions still refresh
    // mutated data immediately (revalidatePath bypasses this window). A
    // signed-out user can soft-navigate back into cached segments for up to
    // 30s — middleware still gates every hard navigation and server request.
    staleTimes: {
      dynamic: 30,
    },
    serverActions: {
