...
export function fillEnvTemplate(template: string, values: Record<string, string>): string {
  return template
    .split("\n")
    .map((line) => {
      // The optional `\r` tolerates a CRLF template (see `parseEnvFile`), and
      // is written back so a file's own line endings survive being filled.
      const match = /^([A-Z][A-Z0-9_]*)=(\r?)$/.exec(line);
      if (!match) return line;
      const value = values[match[1]!];
      return value === undefined || value === "" ? line : `${match[1]}=${value}${match[2]}`;
    })
    .join("\n");
}
...
export function parseEnvFile(contents: string): Record<string, string> {
  ...
  // Split on CRLF as well as LF: a file the user saved from a Windows editor,
  // or a checkout with `core.autocrlf`, otherwise leaves a `\r` on every line,
  // which `(.*)$` cannot match (`.` skips line terminators), and every key
