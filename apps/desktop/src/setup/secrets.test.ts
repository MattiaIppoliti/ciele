...
describe("fillEnvTemplate", () => {
  ...
  it("reads a CRLF file the same as an LF one", () => {
    // A file saved from a Windows editor (or a checkout with core.autocrlf)
    // carries \r\n; without tolerating it every key is dropped silently and
    // the wizard regenerates secrets it should have reused.
  ...
  it("fills a CRLF template and preserves its line endings", () => {
    const filled = fillEnvTemplate("POSTGRES_PASSWORD=\r\nAPP_PORT=3000\r\n", {
      POSTGRES_PASSWORD: "pw",
    });
    expect(filled).toBe("POSTGRES_PASSWORD=pw\r\nAPP_PORT=3000\r\n");
  });
});
