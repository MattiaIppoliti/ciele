 ...
 *
 * One origin serves the marketing site, the console and the widget runtime.
 * The docs used to name a `platform.` subdomain that was never provisioned,
 * and the first cut of this file took them at their word, so Sign in landed
 * on a 404 inside a window with no address bar. A default that does not
 * resolve is worse than no default. If the console ever moves to its own
 * host, this constant is the one line that moves.
 */
