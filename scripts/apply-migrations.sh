...
# predate the ledger), but only on a database that already carries what they
# built; on an empty one the whole chain runs from the start. Every other file
# runs in C-locale filename order, each
...
# Record baseline files as applied without executing them, but only against a
# database that already has what they built. An empty database (a fresh
# self-host install, the CI replay in migrations.yml) has none of it, and
# recording the baseline there would mark every listed file applied without
# ever running it, leaving the rest of the chain to fail against tables that
# were never created. The baseline is a reconciliation device for THIS
# project's live database, not a property of the chain: probe first, and on an
# empty database apply the whole chain from the first file instead.
# `public.assistants` is 0001_init's first table, so its absence is the honest
# test for "nothing has been applied here yet".
#
# MIGRATIONS_BASELINE=record|skip overrides the probe (auto by default).
baseline_mode="${MIGRATIONS_BASELINE:-auto}"
if [[ "$baseline_mode" == "auto" ]]; then
  if [[ "$("${PSQL[@]}" -At -c "select to_regclass('public.assistants') is not null;")" == "t" ]]; then
    baseline_mode=record
  else
    baseline_mode=skip
  fi
fi
...
record)
  ...
  ;;
skip)
  echo "Empty database: applying the full chain from the first migration"
  echo "(${BASELINE_FILE} not recorded, nothing is here to reconcile with)."
  ;;
*)
  echo "Error: MIGRATIONS_BASELINE must be 'record', 'skip' or 'auto' (got '${baseline_mode}')." >&2
  exit 2
  ;;
esac
