...
`apps/web/src/middleware.ts` and `apps/admin/src/middleware.ts` both perform the same ~25-line
Supabase SSR cookie dance (`@supabase/ssr` client, session refresh, redirect-to-login) before
diverging into their own guards. An architecture review (July 2026) flagged the duplication and
proposed a shared `createSupabaseMiddleware(guard)` factory package that both apps — and any future
app — would consume.
...
- **Different trust boundaries, on purpose.** `apps/web` gates tenant Members into an org-scoped
  session that RLS depends on; `apps/admin` gates Ciele staff via an email allowlist in front of a
  **service-role** client that intentionally bypasses that same RLS. The admin README frames its
  gate as a deliberate placeholder for a WorkOS SSO swap (`isPlatformAdmin` is the one function to
  replace). A shared factory would blur the most security-sensitive distinction in the codebase
...
share the same session mechanism (the WorkOS swap in `apps/admin` would remove that premise anyway).
