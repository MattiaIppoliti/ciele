...
blocked), and `renderTable` (opt-in), a directly registered tool whose whole
effect is the **Reply Component** it shows the Visitor, with its arguments
streamed to the client so the component builds up rather than appearing
finished. Ordinary specs are wrapped by `instrumentAction()`, which emits the
