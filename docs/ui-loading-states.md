...
| `(admin)/insights` | `(admin)/insights/loading.tsx` | Header toolbar + date chip + 12-col metric grid + chart (renders inside `insights/layout.tsx`, so the Insights/Exports sub-nav stays) |
| `(admin)/assistants/[id]` | `(admin)/assistants/[id]/loading.tsx` | Initial Assistant editor / overview center column |
| `(admin)/assistants/[id]/<section>` | Each SETUP section's `loading.tsx` | Destination-owned Assistant section fallback; required so sibling navigation replaces the previous section immediately |
