...
dependencies. Every section is a nested route module (`/preview`, `/general`, `/knowledge`, `/flows`,
...
**SETUP nav** (`components/shell/nav.ts` + `components/app-sidebar.tsx`) lists live routes directly:
...
| Flows | `flows` | ✅ live |
| Tools & Skills | `tools` | ✅ live |
| Goals | `goals` | ✅ live |
| Assistant Help Desks | `help-desks` | ✅ live |
| Style | `style` | ✅ live |
| Authentication | `authentication` | ✅ live |
| Publish | `publish` | ✅ live |

> All ten sections are live routes. Unimplemented sections stay out of `SETUP_SECTIONS` until their
> route exists.
...
| Component | File | Role |
|-----------|------|------|
| `AppSidebar` | `components/app-sidebar.tsx` | Global rail; links every live SETUP section |
...
render-only tool's arguments as they are written (AG-UI's `TOOL_CALL_ARGS` shape), and *only* for
`renderTable`, so its Reply Component materializes instead of appearing finished. The fold ignores
...
- **`supabase.ts`**, `createSupabaseDb(client)`: RLS-scoped queries, server-side joins, and RPCs
  (`create_organization`, `accept_invite`, `match_chunks_linked`, `next_improvement_seq`). Vector search with
...
- **Tool registry** (`tools.ts`): a tool is a spec (name, description, zod input schema, step label,
  ...
  *not* routed through `instrumentAction`: its lifecycle, coverage verdict and budget accounting live in
  ...
  (default **off**, egress is opt-in), and the directly registered `renderTable` (default **off**,
  see below). `instrumentAction()` gives every ordinary spec its
...
- **`renderTable`** (`render-tools.ts` + `reply-components.ts`): a directly registered tool whose
  whole effect is the **Reply Component** it puts in front of the Visitor. `renderTableTool()` gives
  it the full `tool-start`/`tool-end` lifecycle, its panel row and its Simplified-thinking narration; the
  part is emitted *and* handed to a turn-owned collector, so the component persists with the answer
  and the Inbox transcript shows what the Visitor saw. Its props are
  ...
  component (`verifier.ts` reads text, and `verifier.test.ts` locks that as a decision), so it must
  *arrange* retrieved data rather than compute a claim; and it must earn its place against
...
- **Retrieval**: `match_chunks_linked(assistant_id, collection_id, query_embedding, k)` cosine
  search, scoped by the **assistant↔source link table** rather than by Collection (#733/#741,
...
| **Tool** (agent-facing capability) | Add a `RuntimeToolSpec` in `packages/agent/src/tools.ts` and, if it should be gateable, a `BUILT_IN_DEFAULTS` entry + the Tools & Skills toggle. `instrumentAction()` supplies lifecycle events, error containment and Thinking-panel progress for ordinary specs; `renderTableTool()` is registered directly because it owns that lifecycle itself. No engine edit is needed. An org can also reach its own HTTP API with no deploy by describing endpoints in its **API integration** catalogue; an MCP-style client would slot in as a tool *provider*, spreading its list into what `buildToolset` returns. |
...
- **AI Teammates** (spec #767, effort 1): the Assistant's internal sibling, org-level agents Members
  ...
  is no second engine: a persona prompt layer, a Knowledge Scope of Library Collections **and
  individual Library Sources** (websites, files, FAQs, unioned into one search by
  `buildCollectionSearcher` and deduplicated back to one top-k), action
...
- **Retrieval hardening**: `searchChunks` tops up partial vector results with lexical matches
  (null-embedding chunks are never masked); `match_chunks_linked` filters `concepts.excluded` in SQL.
