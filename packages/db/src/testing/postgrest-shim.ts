...
export class PgliteRest {
  ...
  constructor(
    ...
    private role: "authenticated" | "service_role" = "authenticated"
  ) {}
  ...
  async rpc(fn: string, args: Record<string, unknown> = {}) {
    try {
      // The production adapter can be backed by either the caller-scoped or
      // service-role client. Mirror that distinction for RPC body checks; the
      // shim deliberately does not otherwise emulate PostgREST grants/RLS.
      await this.pg.query(
      ...
      const overloads = new Map<string, typeof proc.rows>();
      for (const row of proc.rows) {
        const rows = overloads.get(row.functionOid) ?? [];
        rows.push(row);
  ...
  get auth() {
    ...
    return {
      ...
      // Mirrors supabase-js getClaims closely enough for the adapter's
      // identity reads (sub/email) — local JWT verification has no shim
      // equivalent, so the claims are synthesized from the shim user.
      async getClaims() {
        return {
...
export function createPgliteSupabaseClient(
  ...
  const rest = new PgliteRest(pg, user, role);
  return {
