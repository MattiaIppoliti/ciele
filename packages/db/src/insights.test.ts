...
import { getMockDb, DEMO_ORG } from "./index";
import {
  colorizeOverview,
  computeBreakdown,
 ...
 * see CLAUDE.md §8) plus parity tests that assert the assembled Overview and
 * the in-memory Db adapter agree with those functions. The production SQL
 * function `get_insights_overview` mirrors the same contract. Fixtures
 * (builders + FILTER_CASES) are shared with the SQL parity test via
 * ./testing/insights-fixtures.
 */
...
// --- Db seam: the in-memory adapter agrees with the oracle ----------------
