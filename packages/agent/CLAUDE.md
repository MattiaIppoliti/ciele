...
- `render-tools.ts`, the `renderTable` schema and part builder. `tools.ts` registers
  `renderTableTool` directly with the same lifecycle, panel row and Simplified-thinking narration as
  an instrumented action, and its arguments stream to the client as `tool-input-delta` so the
