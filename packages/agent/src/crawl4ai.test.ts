...
describe("buildCrawl4aiJob", () => {
  ...
  it("allows medium self-hosted crawls and clamps at the worker safety ceiling", () => {
    expect(
      deepCrawlParams(buildCrawl4aiJob("https://x.edu", { maxPages: 500 }))
        .max_pages
    ).toBe(500);
    expect(
      deepCrawlParams(buildCrawl4aiJob("https://x.edu", { maxPages: 50_000 }))
        .max_pages
    ).toBe(5_000);
