...
describe("graph-worker HTTP calls", () => {
  ...
  it("runs paged epoch/mark-and-sweep reconciliation", async () => {
    fetchMock
      .mockResolvedValueOnce(jsonResponse({ phase: "index", cursor: null, done: false }))
      .mockResolvedValueOnce(jsonResponse({ phase: "mark", cursor: null, done: false }))
      .mockResolvedValueOnce(jsonResponse({ phase: "deliver", cursor: null, missing_concept_ids: ["c2"] }))
      .mockResolvedValueOnce(jsonResponse({ phase: "sweep", cursor: null }))
      .mockResolvedValueOnce(jsonResponse({ phase: "done", removed: 2, done: true }));
    await expect(resumeConceptReconciliation("col-1")).resolves.toEqual({
      phase: "index",
      cursor: null,
      done: false,
      missingConceptIds: [],
    ...
    await expect(
      indexConceptInventory("col-1", 100),
    ).resolves.toEqual({
      phase: "mark",
      done: false,
      cursor: null,
    ...
    await expect(
      reconcileConceptInventory(
        ...
        null,
        [
          { conceptId: "c1", contentHash: "hash-1" },
          { conceptId: "c2", contentHash: "hash-2" },
        ],
        null,
        true,
      ),
      ...
      missingConceptIds: ["c2"],
    });
    await expect(acknowledgeConceptInventory("col-1")).resolves.toEqual({
      phase: "sweep",
      cursor: null,
      done: false,
      missingConceptIds: [],
    ...
    await expect(
      sweepConceptInventory("col-1", 100),
    ).resolves.toEqual({
      phase: "done",
      cursor: null,
      removed: 2,
    ...
    expect(fetchMock.mock.calls.map(([url]) => url)).toEqual([
      "https://graph.internal/reconcile-v2",
      "https://graph.internal/reconcile-v2",
      "https://graph.internal/reconcile-v2",
      "https://graph.internal/reconcile-v2",
      "https://graph.internal/reconcile-v2",
    ...
    expect(JSON.parse(fetchMock.mock.calls[2]![1].body)).toEqual({
      ...
      desired_done: true,
      desired: [
        { concept_id: "c1", content_hash: "hash-1" },
        { concept_id: "c2", content_hash: "hash-2" },
      ],
    });
  });

  it("keeps the original reconciliation endpoint for an older worker rollout", async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({ removed: 1, missing_concept_ids: ["c2"] }),
    );
    await expect(
      reconcileLegacyConceptInventory("col-1", [
        { conceptId: "c1", contentHash: "hash-1" },
      ]),
    ).resolves.toEqual({ removed: 1, missingConceptIds: ["c2"] });
    expect(fetchMock.mock.calls[0]![0]).toBe("https://graph.internal/reconcile");
  });

  it("detects a worker that predates resumable reconciliation", async () => {
    fetchMock
      .mockResolvedValueOnce(jsonResponse("not found", false, 404))
      .mockResolvedValueOnce(jsonResponse("method not allowed", false, 405));
    await expect(resumeConceptReconciliation("col-1")).resolves.toBe("legacy");
    expect(fetchMock.mock.calls[1]![1]).toMatchObject({ method: "OPTIONS" });
  });

  it("treats a worker with no reconciliation endpoint as unavailable", async () => {
    fetchMock
      .mockResolvedValueOnce(jsonResponse("not found", false, 404))
      .mockResolvedValueOnce(jsonResponse("not found", false, 404));
    await expect(resumeConceptReconciliation("col-1")).resolves.toBeNull();
  });
