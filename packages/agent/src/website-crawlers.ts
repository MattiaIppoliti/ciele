...
export type WebsiteCrawlPollResult =
  ...
  | {
      status: "succeeded";
      pages: CrawledPage[];
      /** Opaque provider cursor; null means this was the last dataset window. */
...
const localAdapter: WebsiteCrawlerAdapter = {
  ...
  async poll({ url, options }) {
    return {
      status: "succeeded",
      pages: await localCrawl(url, options),
      nextCursor: null,
    };
  },
};
  ...
  async poll({ runId, datasetId, url, options, cursor }) {
    ...
    if ((options.maxPages ?? 20) > APIFY_DATASET_BATCH_SIZE || cursor) {
      ...
      return {
        ...
        pages: batch.pages,
        nextCursor:
          batch.nextOffset === null ? null : String(batch.nextOffset),
      };
    }
    return {
      status: "succeeded",
...
export interface WebsiteCrawlCharacteristics {
  ...
  /** Requested page budget needs the managed crawler's paginated dataset. */
  exceedsSelfHostedCap: boolean;
}
...
export function resolveWebsiteCrawlerProvider(
  ...
  if (characteristics.exceedsSelfHostedCap) {
    return capabilities.apifyConfigured
      ? { provider: "apify" }
      : { error: NO_LARGE_CORPUS_PROVIDER };
  }
