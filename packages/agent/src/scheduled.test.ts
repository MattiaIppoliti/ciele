...
vi.mock("./effects", async (importOriginal) => ({
  ...
  drainTurnEffects: mocks.drainTurnEffects,
}));
...
function stubDb(overrides: Partial<Db>): Db {
  return {
    ...
    sweepRuntimeLedgers: vi.fn().mockResolvedValue({
      backgroundJobs: 0,
      conversationTurns: 0,
      turnEffects: 0,
...
beforeEach(() => {
  ...
  mocks.drainTurnEffects.mockReset();
  mocks.drainTurnEffects.mockResolvedValue({
    claimed: 0,
    succeeded: 0,
    failed: 0,
    superseded: 0,
...
describe("finalizeDueCrawls", () => {
  it("keeps draining bounded queue windows until a pass is empty", async () => {
    mocks.drainTurnEffects
    ...
    const report = await finalizeDueCrawls({ db: stubDb({}) });

    expect(mocks.drainTurnEffects).toHaveBeenCalledTimes(2);
  ...
  it("runs bounded runtime-ledger retention on every durable queue tick", async () => {
    const sweepRuntimeLedgers = vi.fn().mockResolvedValue({
      backgroundJobs: 3,
      conversationTurns: 4,
      turnEffects: 5,
      apiIdempotencyKeys: 6,
      sourceGenerations: 7,
    });
    const now = new Date("2026-08-29T12:00:00.000Z");

    const report = await finalizeDueCrawls(
      { db: stubDb({ sweepRuntimeLedgers }) },
      { now }
    );

    expect(sweepRuntimeLedgers).toHaveBeenCalledWith(now.toISOString(), 500);
    expect(report.ledgerSweep).toEqual({
      backgroundJobs: 3,
      conversationTurns: 4,
      turnEffects: 5,
      apiIdempotencyKeys: 6,
  ...
  it("raises an operational error when durable work exceeds the queue-age SLO", async () => {
    ...
    await finalizeDueCrawls(
      {
        db: stubDb({
          getWorkQueueHealth: vi.fn().mockResolvedValue({
            backgroundJobs: {
              ingest: { due: 7, oldestDueAt: "2026-08-29T11:30:00.000Z" },
            },
            turnEffects: { due: 0, oldestDueAt: null },
          }),
        }),
