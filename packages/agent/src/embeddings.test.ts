...
describe("embedding usage metering (#438)", () => {
  ...
  it("bounds provider batches while preserving output order", async () => {
    aiMocks.embedMany.mockImplementation(async ({ values }) => ({
      embeddings: values.map((value: string) => [Number(value)]),
      usage: { tokens: values.length },
    }));
    const recordAiUsage = vi.fn();
    const db = { recordAiUsage } as unknown as Db;
    const texts = Array.from(
      { length: EMBEDDING_BATCH_SIZE * 2 + 1 },
      (_, index) => String(index)
    );

    const result = await embedTextsWithStatus(texts, [], {
      db,
      organizationId: "org-1",
    });

    expect(aiMocks.embedMany).toHaveBeenCalledTimes(3);
    expect(aiMocks.embedMany.mock.calls.map(([call]) => call.values.length)).toEqual([
      EMBEDDING_BATCH_SIZE,
      EMBEDDING_BATCH_SIZE,
      1,
    ]);
