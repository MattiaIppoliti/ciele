...
import { CRAWL4AI_MAX_CRAWL_PAGES } from "./crawl4ai";
import {
  browserCrawlerFor,
...
describe("resolveWebsiteCrawlerProvider, Automatic", () => {
  ...
  it("routes a larger-than-self-hosted crawl to paginated Apify", () => {
    expect(
      resolveWebsiteCrawlerProvider(
        "auto",
        { ...STATIC, exceedsLocalCap: true, exceedsSelfHostedCap: true },
        caps(true, true)
      )
    ).toEqual({ provider: "apify" });
  });

  it("rejects a larger-than-self-hosted crawl when Apify is absent", () => {
  ...
  it("reads traits from a Source's own config fields", () => {
    expect(
      crawlCharacteristicsFromConfig({
        ...
        loginProtected: true,
        waitSecs: 2,
        maxPages: CRAWL4AI_MAX_CRAWL_PAGES + 1,
      })
    ).toEqual({
      fetchFiles: true,
      loginProtected: true,
