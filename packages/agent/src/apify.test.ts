...
describe("buildCrawlInput", () => {
  ...
  it("allows large managed crawls and clamps only at the corpus safety ceiling", () => {
    expect(buildCrawlInput("https://x.edu", { maxPages: 500 }).maxCrawlPages).toBe(500);
    expect(
      buildCrawlInput("https://x.edu", { maxPages: 500_000 }).maxCrawlPages
    ).toBe(100_000);
...
describe("dataset pagination", () => {
  it("reads one bounded dataset window and advances by raw item count", async () => {
    vi.stubEnv("APIFY_API_TOKEN", "test-token");
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify([
          { url: "https://x.edu/a", markdown: "A" },
          { url: "https://x.edu/empty", markdown: "" },
        ]),
    ...
    await expect(
      fetchCrawledPageBatch("dataset-1", "https://x.edu", {
        offset: 100,
        limit: 2,
      })
    ).resolves.toEqual({
      pages: [{ url: "https://x.edu/a", title: "https://x.edu/a", text: "A" }],
      nextOffset: 102,
    });
    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      "https://api.apify.com/v2/datasets/dataset-1/items?clean=true&format=json&offset=100&limit=2"
    );
  });
});
