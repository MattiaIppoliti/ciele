...
describe("buildSystemPrompt (prompt layering)", () => {
  it("lifts the no-addressing-the-user ban for renderTable, when enabled", () => {
...
describe("api_request", () => {
  ...
  it("passes the durable turn operation key to mutating endpoints", async () => {
    const { ctx } = makeContext({
      flow: apiFlow({ method: "POST", url: "https://api.example.com/hook" }),
    });
    ctx.idempotencyKey = "conversation-turn/c-1/r-1/action-0";

    await ACTION_HANDLERS.api_request(ctx);

    expect(egressFetchMock.mock.calls[0][1].headers?.["idempotency-key"]).toBe(
      "conversation-turn/c-1/r-1/action-0"
    );
  });
