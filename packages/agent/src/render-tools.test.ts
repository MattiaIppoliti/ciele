...
/**
 * The renderTable generative-UI tool: its whole effect is a
 * component in front of the Visitor. What is under test is the contract that
 * makes that safe: the props are validated, the part
...
describe("renderTable registration", () => {
  ...
  it("stays unregistered with no part collector wired", () => {
    ...
    // tool is withheld rather than half-wired.
...
describe("renderTable", () => {
  ...
  it("caps rows rather than refusing, so a runaway table still renders", async () => {
    ...
    await run(buildToolset(ctx), "renderTable", { columns: ["n"], rows });
    const part = collected[0];
    expect(part?.type).toBe("component");
    if (part?.type !== "component") throw new Error("Expected table component");
    expect(part.props.rows).toHaveLength(REPLY_COMPONENT_LIMITS.tableRows);
  });
