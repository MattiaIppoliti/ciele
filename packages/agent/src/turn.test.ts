...
describe("streamConversationTurn", () => {
  it("replays a completed turn without duplicating messages or effects", async () => {
    ...
    await db.createFlow(assistant.id, {
      name: "Idempotent improvement",
      description: "I have a refund request",
      actions: ["improvement"],
    });
    const flows = await db.listFlows(assistant.id);
    ...
    const first = await runTurn({
      assistant,
      flows,
      message: "I have a refund request",
      turnId,
    });
    ...
    expect(first.find((event) => event.type === "flow")).toMatchObject({
      flowName: "Idempotent improvement",
    });
    expect(doneEvent(replay).messageId).toBe(doneEvent(first).messageId);
    expect(await db.listMessages(doneEvent(first).conversationId)).toHaveLength(2);
    expect(await db.listImprovements(DEMO_ORG.id)).toHaveLength(
      before.length + 1
    );
  });
...
describe("chat_turn telemetry (ADR-0011)", () => {
  ...
  it("records a failed chat_turn with an error class when the turn throws", async () => {
    ...
    const brokenDb: Db = {
      ...db,
      async commitConversationTurn() {
        throw new TypeError("persist boom");
      },
      async recordRuntimeEvent(event) {
        captured.push(event);
      },
...
describe("daily budget (notify mode)", () => {
  ...
  it("atomically settles and releases an admitted hard-budget turn", async () => {
    const { assistant } = await fixture();
    const exact = await db.createFlow(assistant.id, {
      name: "Budget settlement",
      description: "settle this budget",
      actions: ["custom_message"],
      customMessage: "settled",
    });
    ...
    const watchedDb: Db = {
      ...
      async settleOrgBudgetReservation(id, rows) {
        settled += 1;
        return db.settleOrgBudgetReservation(id, rows);
      },
      async releaseOrgBudgetReservation(id) {
        released += 1;
        return db.releaseOrgBudgetReservation(id);
      },
    };
    try {
      const stream = await streamConversationTurn({
        ...
        organizationId: DEMO_ORG.id,
        subjectType: "visitor",
        subjectId: "visitor-budget-settlement",
        message: "settle this budget",
        signal: new AbortController().signal,
      });
      await new Response(stream).text();
