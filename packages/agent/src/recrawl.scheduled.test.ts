...
import { finalizeDueCrawls, sweepDueRecrawls } from "./scheduled";
 ...
 *
 * Only the network primitives are faked, so this is the end-to-end provider
 * seam. `scheduled.test.ts` covers the tick's batch/lease/report policy;
 * apps/web's cron route tests cover the auth wrapper over these two calls.
 */
describe("scheduled re-crawl provider seam", () => {
  ...
  // Both drains take the Db they act on, so the sweep and the finalizer see the
  // same Sources this test seeds without any service-role indirection.
  const sweep = () => sweepDueRecrawls({ db: getMockDb() });
  const finalize = () => finalizeDueCrawls({ db: getMockDb() });
  ...
  it("runs a scheduled Local re-crawl through the shared start + finalize lifecycle", async () => {
    ...
    expect(await sweep()).toMatchObject({ recrawls: { swept: 1, launched: 1 } });
  ...
  it("does not start a duplicate remote run when the sweep runs twice before finalization", async () => {
    ...
    expect(first).toMatchObject({ recrawls: { swept: 1 } });
    expect(second).toMatchObject({ recrawls: { swept: 0 } });
    expect(startCrawl4aiMock).toHaveBeenCalledOnce();
    expect((await db.getSource(sourceId))?.config).toMatchObject({
      crawlRunId: "task-1",
    });
