...
export interface GraphDocument {
  ...
  text: string;
  /** Stable digest used by desired-set repair to detect changed documents. */
  contentHash?: string;
}
...
type ReconcileResult = {
  ...
  phase?: "index" | "mark" | "deliver" | "sweep" | "done";
  cursor?: string | null;
  removed?: number;
...
async function reconcileRequest(
  ...
  const dataset = datasetForCollection(collectionId);
  return workerRequest<ReconcileResult>(
    "/reconcile-v2",
    { dataset, ...body },
    WRITE_TIMEOUT_MS,
  );
...
export type GraphReconcileState = {
  epoch?: string;
  phase: "index" | "mark" | "deliver" | "sweep" | "done";
  cursor: string | null;
  done: boolean;
...
function mapReconcileState(result: ReconcileResult): GraphReconcileState {
  return {
    ...(result.epoch ? { epoch: result.epoch } : {}),
    phase: result.phase ?? "index",
    cursor: result.cursor ?? null,
    done: result.done === true,
    missingConceptIds: result.missing_concept_ids ?? [],
  };
}

/** Resume the worker-owned state, identifying legacy or unavailable workers. */
export async function resumeConceptReconciliation(
  collectionId: string,
): Promise<GraphReconcileState | "legacy" | null> {
  try {
    return mapReconcileState(
      await reconcileRequest(collectionId, { action: "resume" }),
    );
  } catch (error) {
    if (error instanceof Error && error.message.includes("/reconcile-v2 failed (404)")) {
      return (await legacyReconciliationAvailable()) ? "legacy" : null;
    }
    throw error;
  }
}

/** Probe the legacy route before materializing an unbounded desired inventory. */
async function legacyReconciliationAvailable(): Promise<boolean> {
  const { baseUrl, token } = requireConfig();
  ...
  const detail = redactGraphWorkerSecrets(
    await response.text().catch(() => ""),
  );
  throw new Error(
    `Graph worker /reconcile capability probe failed (${response.status}): ${detail.slice(0, 200)}`,
  );
}

/** Snapshot one bounded page of the worker's current document inventory. */
export async function indexConceptInventory(
  collectionId: string,
  limit: number,
): Promise<GraphReconcileState> {
  const result = await reconcileRequest(collectionId, {
    action: "index",
    limit,
  });
  return mapReconcileState(result);
}

/** Mark one authoritative Concept page and return missing/changed ids. */
export async function reconcileConceptInventory(
  collectionId: string,
  cursor: string | null,
  desired: Array<{ conceptId: string; contentHash: string }>,
  nextCursor: string | null,
  desiredDone: boolean,
): Promise<GraphReconcileState & { missingConceptIds: string[] }> {
  const result = await reconcileRequest(collectionId, {
    action: "mark",
    cursor,
    next_cursor: nextCursor,
    desired_done: desiredDone,
  ...
  return {
    ...mapReconcileState(result),
    missingConceptIds: result.missing_concept_ids ?? [],
  };
}

/** Delete one bounded stale inventory page after every desired page was marked. */
export async function sweepConceptInventory(
  collectionId: string,
  limit: number,
): Promise<GraphReconcileState & { removed: number }> {
  const result = await reconcileRequest(collectionId, {
    action: "sweep",
    limit,
  });
  return { ...mapReconcileState(result), removed: result.removed ?? 0 };
}

/** Acknowledge that every missing-id job from the pending mark page is durable. */
export async function acknowledgeConceptInventory(
  collectionId: string,
): Promise<GraphReconcileState> {
  return mapReconcileState(
    await reconcileRequest(collectionId, { action: "ack" }),
  );
}

/** One-deploy compatibility path for an older worker without `/reconcile-v2`. */
export async function reconcileLegacyConceptInventory(
  collectionId: string,
  desired: Array<{ conceptId: string; contentHash: string }>,
): Promise<{ removed: number; missingConceptIds: string[] } | null> {
  const dataset = datasetForCollection(collectionId);
  let result: ReconcileResult;
  try {
    result = await workerRequest<ReconcileResult>(
      "/reconcile",
      {
        dataset,
        desired: desired.map((item) => ({
    ...
    if (error instanceof Error && error.message.includes("/reconcile failed (404)")) {
      return null;
    }
    throw error;
  }
