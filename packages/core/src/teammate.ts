...
export function teammateSearchesKnowledge(
  teammate: Pick<Teammate, "collectionIds">
): boolean {
  return teammate.collectionIds.length > 0;
}
...
export function teammatePersonaPrompt(
  teammate: Pick<Teammate, "name" | "title" | "roleDescription" | "collectionIds">
): string {
...
export function danglingScopeAlertCopy(
  ...
  return {
    title: `Deleted collection still in a teammate's knowledge: ${collectionName}`,
    detail: `${teammateNames.join(", ")} ${one ? "searches" : "search"} a knowledge collection that no longer exists, so ${one ? "it finds" : "they find"} nothing there. Edit the teammate's knowledge to clear it.`,
  };
}
