...
describe("ceilingAllowsCapability", () => {
  it("lets a granted operation through at or under the ceiling", () => {
    ...
    expect(ceilingAllowsCapability("publish", "publish")).toBe(true);
  });
  ...
  it("refuses org administration at every ceiling", () => {
    for (const ceiling of ["member", "edit", "publish"] as const) {
  ...
  it("refuses a capability it has never heard of", () => {
    ...
    // `publish` by accident: unknown is denied until somebody decides.
    expect(ceilingAllowsCapability("publish", "impersonate")).toBe(false);
    expect(ceilingAllowsCapability("publish", "")).toBe(false);
  });
});
...
describe("mayAcceptSuggestedFix", () => {
  ...
  it("refuses a bypass with no knowledge grant", () => {
    ...
    expect(
      mayAcceptSuggestedFix({ approvalBypass: true }, [grant("improvements")])
    ).toBe(false);
  ...
  it("allows the one combination an admin set on purpose", () => {
    expect(
      mayAcceptSuggestedFix({ approvalBypass: true }, [grant("knowledge")])
    ).toBe(true);
  });
});
