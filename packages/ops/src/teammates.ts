...
export const createTeammateOp = defineOperation({
  ...
  run: async (ctx, input) => {
    ...
    // A scope can name a Collection that is already gone (#769).
...
export const deleteTeammateOp = defineOperation({
  ...
  run: async (ctx, { id }): Promise<void> => {
    ...
    // A retired Teammate searches nothing, so it stops holding a
    // dangling-Collection Alert open on its own (#769).
...
export const startReferralOp = defineOperation({
  ...
  run: async (ctx, input): Promise<{ conversationId: string }> => {
    ...
    await ctx.db.updateConversationMetadata(origin.id, {
      ...origin.metadata,
      referredTo: [
        ...(origin.metadata.referredTo ?? []),
        {
