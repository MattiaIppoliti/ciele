...
describe("channel CRUD and membership", () => {
  it("hydrates list activity in one bounded read and a detail roster without duplicate Teammate reads", async () => {
    const db = getMockDb();
    const { owner, channel } = await seed(db);
    await createChannelOp.run(owner, { name: "Second channel" });
    const windows = vi.spyOn(db, "listChannelMessageWindows");
    const perChannel = vi.spyOn(db, "listChannelMessages");

    await listChannelsOp.run(owner, {});
    expect(windows).toHaveBeenCalledTimes(1);
    expect(perChannel).not.toHaveBeenCalled();

    const tables = vi.spyOn(db, "table");
    await getChannelOp.run(owner, { id: channel.id });
    expect(
      tables.mock.calls.filter(([name]) => name === "teammates"),
    ).toHaveLength(1);
  });
...
/**
 * Being named in a group has to reach the Member outside the group, and it has
 * to arrive **once per group**. The requirement is the cumulation: three
 * colleagues naming you ten times in one thread is one place to go and look, so
 * every case here is about the count staying at one.
 */
describe("mention notifications", () => {
  const mine = async (context: OperationContext, channelId: string) =>
    (await listChannelMentionsOp.run(context, {})).filter(
      (row) => row.channelId === channelId
    );

  it("says nothing until somebody names you", async () => {
    const db = getMockDb();
    const { owner, channel } = await seed(db);
    await addChannelMembersOp.run(owner, { id: channel.id, userIds: [COLLEAGUE] });
    const guest = ctx({ db, userId: COLLEAGUE });

    // Traffic in the group is not a notification: only a name is.
    await postChannelMessageOp.run(owner, {
      id: channel.id,
      message: "some background",
    });
    expect(await mine(guest, channel.id)).toEqual([]);
  });

  it("is one entry per group however many times you were named", async () => {
    const db = getMockDb();
    const { owner, channel } = await seed(db);
    await addChannelMembersOp.run(owner, {
      id: channel.id,
    ...
    const first = await mine(guest, channel.id);
    expect(first).toHaveLength(1);
    expect(first[0].channelName).toBe(channel.name);

    // The same colleague again, then a different one: still one card, because
    // the notification is about the room and not about who shouted.
    await postChannelMessageOp.run(owner, {
      id: channel.id,
      message: "@martina.binacci and again",
    });
    await postChannelMessageOp.run(ctx({ db, userId: OUTSIDER }), {
      id: channel.id,
      message: "@martina.binacci me too",
    });
    expect(await mine(guest, channel.id)).toHaveLength(1);
  });

  it("clears when the Member opens the group", async () => {
    const db = getMockDb();
    const { owner, channel } = await seed(db);
    await addChannelMembersOp.run(owner, { id: channel.id, userIds: [COLLEAGUE] });
    const guest = ctx({ db, userId: COLLEAGUE });
    ...
    // Reading is what clears it; there is no separate dismissal to keep in sync.
    await markChannelReadOp.run(guest, { id: channel.id });
    expect(await mine(guest, channel.id)).toEqual([]);
  });

  it("never notifies the person who wrote the name", async () => {
    const db = getMockDb();
    const { owner, channel } = await seed(db);
    await postChannelMessageOp.run(owner, {
      id: channel.id,
      // Naming yourself is not somebody asking you something.
      message: `@${DEMO_MEMBER.userId} thinking out loud`,
    });
    expect(await mine(owner, channel.id)).toEqual([]);
  });

  it("tells a colleague who is not in the group nothing", async () => {
    const db = getMockDb();
    const { owner, channel } = await seed(db);
    await postChannelMessageOp.run(owner, {
      id: channel.id,
      message: "@u-andrea are you there",
    });
    // Membership is the visibility rule everywhere else in this domain, and a
    // notification that named a group you cannot open would leak its existence.
    expect(
      await mine(ctx({ db, userId: NON_MEMBER }), channel.id)
    ).toEqual([]);
  });
});
