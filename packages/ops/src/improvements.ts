...
import { readInboxSummaryWindowOp } from "./inbox";
 ...
 * Creation stays web-only for now (it fans out into Suggested-Fix drafting);
 * this ships list / detail / update.
 */
...
>({
  ...
  run: async (ctx, { improvementId }): Promise<AcceptedSuggestedFix> => {
    ...
    // knowledge write.
    if (ctx.teammate) {
      if (!mayAcceptSuggestedFix(ctx.teammate, ctx.teammate.grants)) {
        throw new OperationError(
          "invalid_input",
...
export const triageFeedbackOp = defineOperation({
  ...
  run: async (ctx): Promise<FeedbackTriageResult> => {
    ...
    const { conversations: recent } = await readInboxSummaryWindowOp.run(ctx, {
      query: { feedback: "down", from: since },
      // Triage is bounded by design; the oldest feedback waits for the next
      // run rather than turning one unattended job into an unbounded read.
      limit: 1_000,
    });
