...
import {
  ...
  hydrateChannelView,
  loadChannelReadIndex,
  summarizeChannels,
  type ChannelSummary,
  type ChannelView,
} from "./channel-read";

export type { ChannelSummary, ChannelView } from "./channel-read";
...
export const listChannelsOp = defineOperation({
  ...
  run: async (ctx): Promise<ChannelSummary[]> => {
    ...
    if (!userId) return [];
    const index = await loadChannelReadIndex(ctx);
    const mine = index.channels.filter((channel) =>
      isChannelMember(index.participantsByChannel.get(channel.id) ?? [], userId)
    );
    return summarizeChannels(
      ctx,
      mine,
      index.participantsByChannel,
      userId,
    );
  },
});

/** One group where somebody named this Member and they have not read it yet. */
export interface ChannelMention {
  channelId: string;
  channelName: string;
  /** When the group last moved, so the shell can order its cards. */
  at: string;
}

/** The groups where this Member has an unread mention, one entry per group. */
export const listChannelMentionsOp = defineOperation({
  ...
  run: async (ctx): Promise<ChannelMention[]> => {
    ...
    if (!userId) return [];
    const index = await loadChannelReadIndex(ctx);
    const candidates = index.channels.filter((channel) => {
      const seat = (index.participantsByChannel.get(channel.id) ?? []).find(
        (participant) => participant.userId === userId,
      );
      return Boolean(
        seat && (!seat.lastReadAt || channel.updatedAt > seat.lastReadAt),
      );
    });
    const summaries = await summarizeChannels(
      ctx,
      candidates,
      index.participantsByChannel,
      userId,
    ...
    return summaries
      .filter((summary) => summary.unread.mentionsYou)
      .map(({ channel }) => ({
...
export const getChannelOp = defineOperation({
  ...
  run: async (ctx, { id }): Promise<ChannelView> => {
    ...
    return hydrateChannelView(
      ctx,
      channel,
      participants,
      canManageChannel(channel, viewerOf(ctx)),
    );
  },
});
...
export const postChannelMessageOp = defineOperation({
  ...
  run: async (ctx, input): Promise<PostedChannelMessage> => {
    ...
    const { teammates, roster } = await hydrateChannelRoster(ctx, participants);
    const live = new Set(
      teammates
...
async function markRead(
  ...
  await ctx.db
    .table("teammateChannelParticipants")
    .update(seat.id, { lastReadAt: readAt });
}
...
export const markChannelReadOp = defineOperation({
  ...
  run: async (ctx, { id }): Promise<void> => {
    const actor = requireActor(ctx);
    const { channel, participants } = await requireChannel(ctx, id);
    await markRead(ctx, participants, actor, channel.updatedAt);
  },
});
...
export const listOrgChannelsOp = defineOperation({
  ...
  run: async (ctx): Promise<ChannelSummary[]> => {
    ...
    return summarizeChannels(
      ...
      index.channels,
      index.participantsByChannel,
      // An admin reading somebody else's thread has no unread count of their
      // own in it, and inventing one would mark it read for them.
      "",
    );
  },
});
