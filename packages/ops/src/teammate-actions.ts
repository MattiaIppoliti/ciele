 ...
 * - **Everything publish-and-above.** Nothing here declares `publish`, and the
 *   maximum ceiling is `edit`, so those actions are never offered.
 *
...
const IMPROVEMENTS: TeammateActionSpec[] = [
  ...
  },
  {
    ...
    requires: (actor) => mayAcceptSuggestedFix(actor, actor.grants),
    description:
