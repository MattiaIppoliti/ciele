-- Insights Overview: an RLS-invoker read model. The browser never receives
-- raw conversations/messages; this function returns only metrics and bounded
-- time series. Keep it fresh: ADR-0010 intentionally defers materialized
-- rollups until measurements justify their staleness.

create index if not exists conversations_assistant_created_at_idx
  on public.conversations (assistant_id, created_at);
create index if not exists messages_conversation_role_feedback_idx
  on public.messages (conversation_id, role, feedback);
...
filtered_messages as (
  select m.conversation_id, m.role, m.feedback, m.created_at
...
message_bucket_counts as (
  select
    ...
    count(*) filter (where role = 'assistant')::integer as ai_answers,
...
stats as (
  select
    ...
    (select count(*)::integer from filtered_messages where role = 'assistant') as ai_answers,
...
channel_names as (
  ...
  where s.kind = 'website' and a.organization_id = p_organization_id
  group by 1
),
channel_ranked as (
...
options as (
  select jsonb_build_object(
    'roles', coalesce((select jsonb_agg(role order by role) from (select distinct user_role as role from all_conversations where user_role is not null and user_role <> '') roles), '[]'::jsonb),
    'channels', coalesce((select jsonb_agg(jsonb_build_object('value', host, 'label', name || ' (' || host || ')') order by name, host) from channel_names where host is not null and (p_assistant_id is null or exists (select 1 from public.sources s join public.knowledge_collections kc on kc.id = s.collection_id where s.kind = 'website' and kc.assistant_id = p_assistant_id and nullif(regexp_replace(split_part(regexp_replace(coalesce(s.config ->> 'url', ''), '^https?://', '', 'i'), '/', 1), '^www\.', '', 'i'), '') = channel_names.host))), '[]'::jsonb)
  ) as value
)
select jsonb_build_object(
...
revoke execute on function public.get_insights_overview(uuid, date, date, text, text, text, text, text, text) from public, anon;
grant execute on function public.get_insights_overview(uuid, date, date, text, text, text, text, text, text) to authenticated;
