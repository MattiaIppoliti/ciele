-- Allow the Inbox Feedback filter to find explicit neutral reactions, distinct
-- from conversations that have not received feedback.
    ...
    and (
      ...
      or (
        ...
        and exists (
          select 1 from public.messages m
          where m.conversation_id = c.id
            and m.feedback_reaction = 'neutral'
        )
      )
    )
    and (
