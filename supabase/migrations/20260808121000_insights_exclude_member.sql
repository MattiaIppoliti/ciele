-- Insights accounting for proactive Notifications (#546) while excluding
-- staff conversations (#668). This migration deliberately recreates the
-- latest notification-aware function so it does not regress that accounting.
--
-- A Notification is an Assistant message nobody asked for, so counting it like an
-- answer would have turned switching proactive flows on into a metrics change
-- dressed up as a product change. Three rules, applied here and mirrored in the
-- oracle (packages/core/src/insights.ts, ADR-0010):
--
--   1. A Notification is not an AI answer. It leaves `aiAnswers`,
--      `answersPerConversation` and the "AI answers" series untouched.
--   2. A Conversation that contains nothing but Notifications is not a
--      Conversation here. Excluded at `all_conversations`, so total, resolution
--      rate, unique users and every breakdown inherit it from one place. One
...
-- from having to ship whole message bodies just to ask this one question.
alter table public.messages
  add column if not exists proactive boolean
    generated always as (content @> '[{"type": "notification"}]'::jsonb) stored;

create index if not exists messages_conversation_proactive_idx
  on public.messages (conversation_id, proactive);
