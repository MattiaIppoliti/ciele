-- Insights from the pre-flight (spec ciele-org#948, ticket ciele-org#956).
--
-- Three changes to `get_insights_overview`, each mirroring a derivation that
-- now exists in `packages/core/src/insights.ts`. The parity test
-- (`packages/db/src/testing/insights.parity.test.ts`) runs this function and
-- that oracle over one fixture set and asserts they agree, which is the only
-- thing that keeps two implementations of the same KPI honest (ADR-0010).
--
-- 1. "Languages spoken" counts the language the Visitor **wrote in**, read
--    from the pre-flight, falling back to the browser locale where no
--    pre-flight ran. The card was reporting a header.
-- 2. Implicit satisfaction: among conversations nobody rated, the share that
--    ended calm.
-- 3. Escalation-intent rate: the share of conversations where the Visitor
--    asked for a person at least once, whether or not they escalated.
--
-- Both new figures are null rather than zero when nothing carries the signal,
-- because a card reading 0% on a week with no data is a card that lies.
...
as $$
with
-- Role options remain org-wide but come from the incrementally maintained
-- facet read model. The request never probes historical Conversations or
-- Messages just to populate a dropdown.
role_options as (
  select value as role
  from public.admin_read_facets
  where organization_id = p_organization_id
    and facet = 'insights_role'
    and refs > 0
),
window_conversations as (
  select
    c.id,
    c.assistant_id,
    ...
    -- The language the Visitor wrote in (#956), with the browser locale as the
    -- fallback so a Conversation the pre-flight never saw counts as it did
    -- before. The oracle in `packages/core/src/insights.ts` reads the same two
    -- in the same order; the parity test is what keeps them that way.
    coalesce(
      nullif(c.metadata -> 'preflight' ->> 'spokenLanguage', ''),
      c.metadata ->> 'language'
    ) as language,
    (c.metadata -> 'preflight' ->> 'escalationIntent')::boolean as escalation_intent,
    (c.metadata -> 'preflight' ->> 'endedAtFrustration')::integer as ended_at_frustration,
    (c.metadata -> 'preflight') is not null as has_preflight,
  ...
  where a.organization_id = p_organization_id
    -- Bound the expensive Message existence checks before they run. The old
    -- shape computed feedback/notification facts for the Organization's full
    -- history and discarded old rows in the next CTE.
...
-- Feedback and notification classification share one grouped Message pass for
-- the requested Conversation window. The previous shape issued up to four
-- correlated probes per row before any product filter was applied.
window_message_facts as (
  select
    m.conversation_id,
    count(*)::integer as message_count,
    bool_or(m.feedback = 1) as has_positive,
    bool_or(m.feedback = -1) as has_negative,
    bool_or(not m.proactive) as has_non_proactive
  from public.messages m
  join window_conversations c on c.id = m.conversation_id
...
all_conversations as (
  select
    c.*,
    case
      when coalesce(f.has_positive, false) then 1
      when coalesce(f.has_negative, false) then -1
      else 0
    end as feedback,
    (
      coalesce(f.message_count, 0) > 0
      and not coalesce(f.has_non_proactive, false)
    ) as notification_only
  from window_conversations c
  left join window_message_facts f on f.conversation_id = c.id
),
...
stats as (
  select
    ...
    -- Implicit satisfaction (#956): among conversations nobody rated, the
    -- share that ended calm. Rated ones are excluded rather than counted
    -- calm, because the Answer Rating card already reports them.
    (select count(*)::integer from filtered_conversations fc
      where fc.ended_at_frustration is not null
        and not exists (
          select 1 from filtered_messages fm
          where fm.conversation_id = fc.id and fm.feedback in (1, -1)
        )) as unrated_with_signal,
    (select count(*)::integer from filtered_conversations fc
      where fc.ended_at_frustration is not null
        and fc.ended_at_frustration <= 1
        and not exists (
          select 1 from filtered_messages fm
          where fm.conversation_id = fc.id and fm.feedback in (1, -1)
        )) as calm_endings,
    -- Escalation intent (#956): once per conversation, over the ones that
    -- carry a pre-flight. A conversation the shadow never ran on is no
    -- evidence either way, so it is out of the denominator rather than in it
    -- as a negative.
    (select count(*)::integer from filtered_conversations where has_preflight) as with_preflight,
    (select count(*)::integer from filtered_conversations where escalation_intent) as wanted_human
),
languages as (
  select coalesce(
...
options as (
  select jsonb_build_object(
    ...
    'roles', coalesce((select jsonb_agg(role order by role) from role_options), '[]'::jsonb),
...
select jsonb_build_object(
  'stats', jsonb_build_object(
    ...
    'implicitSatisfaction', case when stats.unrated_with_signal > 0
      then round((stats.calm_endings::numeric / stats.unrated_with_signal) * 100)
      else null end,
    'escalationIntentRate', case when stats.with_preflight > 0
      then round((stats.wanted_human::numeric / stats.with_preflight) * 100)
      else null end
  ),
  'chart', chart.value,
