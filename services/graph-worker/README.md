...
| `POST /reconcile` | `{ dataset, desired }` | Legacy one-shot repair contract retained for one rolling-deploy window |
| `POST /reconcile-v2` | `{ dataset, action, desired?, cursor?, next_cursor?, desired_done?, limit? }` | Resumable `index` → `mark` → `deliver`/`ack` → `sweep` repair; each request advances at most one bounded page |
...
V2 reconciliation never materializes a Collection-sized desired set: both
Cognee inventory and authoritative Concepts move through 100-row cursor pages.
Persistent phase and cursor state let later cron invocations resume an
interrupted pass. Missing IDs remain pending until the app acknowledges durable
jobs. The legacy endpoint remains only for rolling compatibility.
