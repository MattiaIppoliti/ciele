...
- `limits.max_pages: 5000`: the self-hosted safety ceiling, as defense in depth
  behind the adapter's own clamp. Larger corpora route to Apify's paginated
  dataset ingestion; `limits.wall_clock_s: 600`, per-crawl deadline;
